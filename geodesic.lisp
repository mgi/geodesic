...
(defun init-alpha1 (lat1 lat2 dlon beta1 beta2 z1)
...
(defun indirect (lat1 lat2 dlon)
  (multiple-value-bind (lat1 lat2) (normalize-latitudes lat1 lat2)
    (let* ((dlon (normalize dlon))
           ...
           (alpha1 (init-alpha1 lat1 lat2 dlon beta1 beta2 z1))
           (delta-dlon 1))
      ;; First: find correct alpha1
      (loop repeat 30
            while (> (abs delta-dlon) 1e-7)
            do (multiple-value-bind (alpha0 sigma1 omega1 alpha2 sigma2 omega2 k2 epsilon)
                 ...
                 (multiple-value-bind (a3 c3) (ac-3 epsilon)
                   (let* ((i3-sigma1 (i sigma1 a3 c3))
                     ...
                     (setf delta-dlon (- dlon lon12)
                           alpha1 (+ alpha1 (delta-alpha1 (- dlon lon12) m12 alpha2 beta2)))))))
      ;; Second: resolve s12
